...
This repository contains a collection of Jupyter notebooks demonstrating data access, analysis, and visualization workflows for the SenNet Data Sharing Portal. The SenNet (Cellular Senescence Network) consortium provides high-resolution, multi-modal datasets including spatial transcriptomics, imaging, and single-cell RNA sequencing data from diverse human and mouse tissues. These notebooks showcase practical applications of spatial analysis across computational biology, with a focus on tissue architecture, cell-cell interactions, and disease-related cellular changes. Whether you're discovering datasets, performing gene expression analysis, or integrating multi-modal imaging data, these demos provide accessible entry points into senescence research.

## Notebooks Overview

Click to navigate to the description of each of the notebooks below:
...
<details closed><summary>Setup environment</summary><p>
...
#### Step 3: Install packages
    ...
    pip install jupyter "notebook>=7" numpy pandas "pyarrow<=20.0.0" 'vitessce[all]' scanpy umap-learn scipy scikit-learn matplotlib globus-cli atlas-consortia-clt requests tifffile imagecodecs igraph leidenalg harmonypy celltypist

or

    # For environment B: demo-affine-transform.ipynb
    pip install jupyter "notebook>=7" "numpy<2.0" pandas "pyarrow<=20.0.0" scanpy umap-learn scipy scikit-learn matplotlib requests fsspec zarr xarray "distributed<=2023" kerchunk tifffile imagecodecs stardist==0.8.5 tensorflow==2.14 igraph leidenalg anndata dask aiohttp==3.8.6
...
The task is to find Donor metadata associated with each of the SenNet datasets of interest. The SenNet Data Sharing Portal (https://data.sennetconsortium.org/) is used to query IDs of all kidney histology datasets generated by UConn Health TMC, resulting in a list of 43 dataset **`sennet_id`**, e.g., `SNT224.LSLK.429`. The list was downloaded in TSV file format. The downloaded TSV file is loaded with Python, then metadata is retrieved via API for each SenNet ID of interest and compiled into a table.
...
Cell type annotation is a cornerstone of spatial transcriptomics analysis. It allows us to map tissue architecture and identify functional zones (e.g., islets of Langerhans vs. exocrine tissue), compare cellular composition across donors, and generate hypotheses about cell-cell interactions, signaling pathways, and disease mechanisms. Here are some key questions this notebook helps address:
...
Features of the Notebook include code to download the annotated dataset for analysis, precomputed UMAP layout of cell types, summary statistics and barplots of cell type distributions, metadata filtering and subgroup comparisons, marker summary dot plots and violin plots.
...
The notebooks were developed and curated by:

- **Dr. Sergii Domanskyi** - UConn Health Tissue Mapping Center (TMC) (Dr. George Kuchel), U54AG075941.
- **Samuel T. Peters** - University of Minnesota Tissue Mapping Center (Dr. Laura J. Niedernhofer), U54AG076041.

## Acknowledgments and Funding

These demo notebooks were developed by The Jackson Laboratory as part of UConn Health TMC and University of Minnesota TMC, both of the [SenNet Consortium](https://sennetconsortium.org/), which is supported by the National Institute on Aging (NIA) and the Common Fund of the National Institutes of Health (NIH). The SenNet Consortium aims to create a comprehensive atlas of cellular senescence across the human body and establish a data and resource sharing portal for the research community.

We acknowledge the contributions of all consortium members, tissue mapping centers, and partner institutions who have generated and curated the datasets featured in these notebooks. The interactive visualization tools and data infrastructure supporting these demonstrations were enabled by the prior work of the Vitessce team and the broader open-source bioinformatics community.

For more information, visit the [SenNet Data Sharing Portal](https://data.sennetconsortium.org/) and the official [SenNet Consortium website](https://sennetconsortium.org/).
